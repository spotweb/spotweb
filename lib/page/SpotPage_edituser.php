...
class SpotPage_edituser extends SpotPage_Abs {
	...
	function render() {
		...
		if ($spotUser === false) {
			$formMessages['errors'][] = vsprintf(_('Gebruiker kan niet gevonden worden'), $spotUser['username']);
		...
		if ((!empty($formAction)) && (empty($formMessages['errors']))) {
			...
			if ($spotUser['userid'] == SPOTWEB_ANONYMOUS_USERID) {
				$formMessages['errors'][] = vsprintf(_('Anonymous user kan niet bewerkt worden'));
				$editResult = array('result' => 'failure');
			} # if

			# sta niet toe, dat de admin user gewist wordt
			if (($spotUser['userid'] <= SPOTWEB_ADMIN_USERID) && ($formAction == 'delete')) {
				$formMessages['errors'][] = vsprintf(_('admin en anonymous user kunnen niet verwijderd worden'));
				$editResult = array('result' => 'failure');
			} # if
		} # if
					...
					if (empty($formMessages['errors'])) {
						...
						if (isset($this->_editUserForm['grouplist'])) {
							...
							if (count($groupList) < 1) {
								$formMessages['errors'][] = vsprintf(_('Een gebruiker moet in minstens een groep zitten'));
								$editResult = array('result' => 'failure');
							} else {
