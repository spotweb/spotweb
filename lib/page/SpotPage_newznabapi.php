...
class SpotPage_newznabapi extends SpotPage_Abs {
    ...
    function search($outputtype) {
        ...
        */
        if ($this->_params['t'] == "t" || $this->_params['t'] == "tvsearch") {
            ...
            # second search on tvdbid if present
            if (($found == false) and ($this->_params['tvdbid'] != "")) {
                # validate input
                if (!preg_match('/^[0-9]{1,6}$/', $this->_params['tvdbid'])) {
                    $this->showApiError(201);
                    return ;
                } # if
                /*
                * Actually retrieve the information from TVMaze, based on the
                * tvdb ID passed by the API
                */
                $svcMediaInfoTvmaze = new Services_MediaInformation_Tvmaze($this->_daoFactory->getCacheDao());
                $svcMediaInfoTvmaze->setSearchid($this->_params['tvdbid']);
                $tvInfo = $svcMediaInfoTvmaze->retrieveInfo();
                $svcMediaInfoTvmaze->setSearchName ( "thetvdb" ); # Indicate thetvdb usage
                $found = $tvInfo -> isValid();
            }
            # third search on rid (rageid) if present
            if (($found == false) and ($this->_params['rid'] != "")) {
                ...
                * tvrage id passed by the API
                */
                $svcMediaInfoTvmaze = new Services_MediaInformation_Tvmaze($this->_daoFactory->getCacheDao());
                $svcMediaInfoTvmaze->setSearchid($this->_params['rid']);
                $tvInfo = $svcMediaInfoTvmaze->retrieveInfo();
                $svcMediaInfoTvmaze->setSearchName ( "tvrage" ); # Indicate tvrage usage
                $found = $tvInfo -> isValid();
            }
            # fourth search on q (showname) if present
            ...
            # fifth, no search information present, set emtpy
            if ($found == false) {
                if ((!empty($this -> _params['tvmazeid'])) or (!empty($this -> _params['rid'])) (!empty($this -> _params['tvdbid']))) {
    ...
    */
    ...
    function caps() {
        ...
        $tvsearch->setAttribute('supportedParams', 'q,rid,tvmazeid,tvdbid,season,ep');
