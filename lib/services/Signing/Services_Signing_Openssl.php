...
class Services_Signing_Openssl extends Services_Signing_Base
{
    ...
    public function createPrivateKey($sslCnfPath)
    {
        ...
        $opensslPrivKey = openssl_pkey_new(['private_key_bits' => 1024, 'config' => realpath($sslCnfPath)]);
     ...
     */
    public function _getOidElementLength($component)
    {
        // Code copied from
        //   http://chaosinmotion.com/wiki/index.php?title=ASN.1_Library
        if ($component < 0) {
            return 10;
        }               // Full 64 bits takes 10*7 bits to encode
        $l = 1;
        for ($i = 1; $i < 9; $i++) {
            $l <<= 7;
            if ($component < $l) {
     ...
     * Helper function for parsing ASN.1
     */
    public function _encodeObjectId($vals)
    {
        $return = [];
     ...
     * Helper function for parsing ASN.1
     */
    public function seclibToOpenSsl($pubKey)
    {
        /*
