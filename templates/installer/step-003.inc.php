...
<table summary="PHP settings">
    ...
    <tr>
        ...
        <td>
            ...
            <input type='hidden' id='portfield' name='nntpform[port]' value='<?php echo $form['port']; ?>' />
    ...
    <tr id='customnntpport' style='display: none;'>
        <td> port</td>
        <td>
            <input type='text' length='3' id='portfld' name='nntpform[port]' onchange='PortChanged(this.value);' value='<?php echo htmlspecialchars($form['port']); ?>' />
        </td>
    </tr>
...
function SelectionChanged() {
    ...
    var x = document.getElementById('customnntpport');
    x.style.display = (res[0] == 'custom') ? '' : 'none';
    ...
    window.onload = function () {
        var val = window.document.getElementById('portfld').value;
        window.document.body.onload = SelectionChanged(); PortChanged(val);
    }
</script>
