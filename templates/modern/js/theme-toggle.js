(function () {
  ...
  function init() {
    ...
    if (toolbar && !document.getElementById("themeCurrent")) {
      ...
      themeWrap.innerHTML =
        '<ul><li><p><a id="themeCurrent">Theme</a></p>' +
        "<ul>" +
    ...
    if (toolbar && !document.getElementById("viewCurrent")) {
      ...
      viewWrap.innerHTML =
        '<ul><li><p><a id="viewCurrent">View: ' +
        (viewPref === "table" ? "Table" : "Cards") +
        "</a></p>" +
        "<ul>" +
      ...
      function setViewCookie(v) {
        try {
          document.cookie = "spotweb_view=" + v + "; path=/; max-age=" + 60 * 60 * 24 * 365;
        } catch (e) {}
      }
      document.getElementById("viewCards").onclick = function () {
        try {
          localStorage.setItem("spotweb_view", "cards");
        } catch (e) {}
        setViewCookie("cards");
      ...
      document.getElementById("viewTable").onclick = function () {
        try {
          localStorage.setItem("spotweb_view", "table");
        } catch (e) {}
        setViewCookie("table");
        var u = new URL(location.href);
      ...
      // Sync cookie from storage if missing. When no cookie, use default 'table' so we don't
      // resurrect stale 'cards' from localStorage (e.g. from before default was changed).
      try {
        if (!document.cookie.match(/(?:^|; )spotweb_view=/)) {
          var prefView = "table";
          try {
            localStorage.setItem("spotweb_view", prefView);
          } catch (e) {}
    ...
    try {
      ...
      if (!u.searchParams.has("view")) {
        ...
        if (prefView) {
          u.searchParams.set("view", prefView);
