    ...
    foreach ($comments as $comment) {
        if ($comment['verified']) {
            ...
            if ($comment['spotrating'] == 0) {
                ...
                echo ' class="poster"';
            } ?>><?php if ($show_avatars) { ?><img class="commentavatar" src='<?php echo $tplHelper->makeCommenterImageUrl($comment); ?>'><?php } ?><strong> <?php echo $rating; ?><?php echo sprintf(_('Posted by %s'), '<span class="user">'.$comment['fromhdr'].'</span>'); ?>
					...
					<br />
					<?php if ($commentIsModerated) {
                echo _('This comment is moderated').'<br /><br />';
            } ?>
