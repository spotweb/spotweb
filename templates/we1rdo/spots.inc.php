	...
	foreach($spots as $spot) {
		...
		if ($spot['stamp'] > 1290578400 ) {
			...
			if (!empty($spot['sabnzbdurl'])) {
				if ($tplHelper->hasBeenDownloaded($spot)) {
					echo "<td><a class='sabnzbd-button succes' href='" . $spot['sabnzbdurl'] . "' title='Add NZB to SabNZBd queue (you already downloaded this spot)'> </a></td>";
				} else {
					echo "<td><a class='sabnzbd-button' href='" . $spot['sabnzbdurl'] . "' title='Add NZB to SabNZBd queue'> </a></td>";	
				} # else
			} # if
		} else {
			if ($settings['show_nzbbutton']) {
