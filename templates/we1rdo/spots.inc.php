...
<?php
	...
	foreach($spots as $spot) {
		...
		"<td class='title " . "bg1" . " " . $newSpotClass . " ". $tipTipClass . "'><a data-cats='" . $catData. "' onclick='openSpot(this,\"".$spot['spoturl']."\")' href='".$spot['spoturl']."' title='" . $spot['title'] . "' class='spotlink'>" . $reportSpam . $rating . $markSpot . $spot['title'] . "</a></td>";
				
		if ($show_editspot_button) {
			echo "<td class='editspot " . "bg1" . " " . $newSpotClass . " ". $tipTipClass . "'>";
		...
		if ($show_watchlist_button) {
			echo "<td class='watch " . "bg1" . " " . $newSpotClass . " ". $tipTipClass . "'>";
		...
		if ($show_comments) {
			echo "<td class='comments " . "bg1" . " " . $newSpotClass . " ". $tipTipClass . "'><a onclick='openSpot(this,\"".$spot['spoturl']."\")' class='spotlink' href='" . $spot['spoturl'] . "#comments' title=\"" . sprintf(_("%d comments on '%s'"), $spot['commentcount'], $spot['title']) . "\">" . $commentCountValue . "</a></td>";
		} # if
		...
		echo "<td class='genre " . "bg1" . "'><a href='" . $spot['subcaturl'] . "' title='" . sprintf(_('Search spot in category %s'), $spot['catdesc']) . "'>" . $spot['catdesc'] . "</a></td>" .
			 "<td class='poster " . "bg1" . "'><a href='" . $spot['posterurl'] . "' title='" . sprintf(_('Search spot from %s'), $spot['poster']) . "'>" . $markSpot . $spot['poster'] . "</a></td>" .
			 "<td class='date " . "bg1" . "' title='" . $dateTitleText . "'>" . $tplHelper->formatDate($spot['stamp'], 'spotlist') . "</td>";

		if ($show_filesize) {
			echo "<td class='filesize " . "bg1" . "'>" . $tplHelper->format_size($spot['filesize']) . "</td>";
		}
		
		# only display the NZB button from 24 nov or later
		if ($spot['stamp'] > 1290578400 ) {
			if ($show_nzb_button) {
				echo "<td class='nzb " . "bg1" . " " . $newSpotClass . " ". $tipTipClass . "'><a href='" . $tplHelper->makeNzbUrl($spot) . "' title ='" . _('Download NZB (n)') . "' class='nzb'>NZB";
			...
			if ($show_multinzb_checkbox) {
				...
				echo "<td class='multinzb " . "bg1" . " " . $newSpotClass . " ". $tipTipClass . "'>";
			...
			if (!empty($spot['sabnzbdurl'])) {
				if ($spot['hasbeendownloaded']) {
					echo "<td class='sabnzbd " . "bg1" . " " . $newSpotClass . " ". $tipTipClass . "'><a onclick=\"downloadSabnzbd('".$spot['id']."','".$spot['sabnzbdurl']."','" . $spot['nzbhandlertype'] . "')\" class='sab_".$spot['id']." sabnzbd-button succes' title='" . _('Add NZB to SABnzbd queue (you already downloaded this spot) (s)') . "'> </a></td>";
				} else {
					echo "<td class='sabnzbd " . "bg1" . " " . $newSpotClass . " ". $tipTipClass . "'><a onclick=\"downloadSabnzbd('".$spot['id']."','".$spot['sabnzbdurl']."','" . $spot['nzbhandlertype'] . "')\" class='sab_".$spot['id']." sabnzbd-button' title='" . _('Add NZB to SABnzbd queue (s)'). "'> </a></td>";
				} # else
			} # if
		} else {
			if ($show_nzb_button) {
				echo "<td class='nzb " . "bg1" . " " . $newSpotClass . " ". $tipTipClass . "'> &nbsp; </td>";
			} # if
			
			# display (empty) MultiNZB td
			if ($show_multinzb_checkbox) { 
				echo "<td class='multinzb " . "bg1" . " " . $newSpotClass . " ". $tipTipClass . "'> &nbsp; </td>";
			}

			# display the sabnzbd button
			if (!empty($spot['sabnzbdurl'])) {
				echo "<td class='sabnzbd " . "bg1" . " " . $newSpotClass . " ". $tipTipClass . "'> &nbsp; </td>";
			} # if
		} # else
